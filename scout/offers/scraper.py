...
# NOTE: data/ lives at the repo root (shared with demand_feed_main.py's own
# _DATA_DIR), not next to this file — this module lives 2 directories deep
# (scout/offers/scraper.py), so __file__'s parent is NOT the repo root here.
# parents[2] = scout/offers -> scout -> repo root. Kept as a plain __file__-
# relative computation (not an import from demand_feed_main) specifically to
# avoid a circular import: demand_feed_main.py imports from this module at
# ITS top level.
_REPO_ROOT = pathlib.Path(__file__).resolve().parents[2]
PAYOUT_CACHE_FILE = os.path.join(_REPO_ROOT, "data", "payout_cache.json")
...
log = logging.getLogger("offer_scraper")
...
def clean_offers(offers: list, ms_index: dict = None) -> list:
    ...
    _snapshot = _REPO_ROOT / "data" / "offers_latest.json"
