...
def test_threshold_history_falls_through_to_llm():
    ...
    try:
        ...
        # A bare deterministic hit would be exactly ("get_threshold_history",) with no args.
        ...
        err = str(e)
        # 401 from Anthropic proves the LLM path was reached — the smoke env has no valid
        # API key, so an auth error here is expected and confirms deterministic router removal.
