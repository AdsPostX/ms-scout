...
| Revenue baseline (PR 24b) | `_query_revenue_baseline(ch)` in `scout_agent.py` | `get_revenue_health()` agent tool + Pulse `_pulse_signal_revenue_baseline()` | 8-week same-weekday median. Fires when actual < 70% of expected. Returns `None` (no anomaly) or dict. `_pulse_signal_revenue_baseline()` in `scout_bot.py` wraps shared query. Threshold: `signals.revenue_baseline_tolerance_pct` in `scout_thresholds.json`. |
...
**[Future — PR 24b follow-up] Revenue baseline doesn't segment AT&T separately** — AT&T revenue dominates platform totals; a soft day for small publishers can be masked by AT&T. Per-publisher baseline (top 5 publishers) would surface segmented anomalies. PR 25 candidate. Single `_query_revenue_baseline_by_publisher(ch)` function could serve both Pulse and agent.
