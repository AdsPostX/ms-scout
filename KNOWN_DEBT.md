...
Fix (superseded — both values are now flipped, see above): flip both values in `config/scout_thresholds.json` and watch the next live digest run before calling it done.
