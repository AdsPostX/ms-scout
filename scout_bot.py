            ...
            while True:  # inner poll loop
                ...
                try:
                    ...
                    if not results:
                        ...
                        firing_names = {s.alert_name for s in _ar.current_state()}
                    ...
                    if os.getenv("SCOUT_ENV", "development") != "production":
                        channel = os.getenv("SCOUT_SHADOW_CHANNEL", "#scout-qa")
                    web_client = WebClient(token=os.getenv("SLACK_BOT_TOKEN", ""))
                    web_client.chat_postMessage(channel=channel, text=fallback, blocks=blocks)

                    _ar.mark_firing(alert_name, {"slot": slot, "count": len(results)})
