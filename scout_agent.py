...
19b. REVENUE HEALTH — "how is revenue", "revenue check", "are we on pace", "revenue health", "is revenue low today", "revenue vs baseline", "how did we do yesterday", "revenue vs historical"
    → get_revenue_health().
    Compares yesterday's total platform revenue vs the 8-week same-weekday median. Shows actual, expected, % of expected.
    Lead with the status emoji (✅ on pace / 🟠 soft / 🔴 significantly below). Bold the key numbers. End with diagnostic prompts if below baseline.
